...
public class EanETLGeoRoute extends SpringRouteBuilder {
    ...
    public void configure() throws Exception {
        ...
        BeanIODataFormat geoAirportsDataFormat = new BeanIODataFormat("classpath:beanio/mappings.xml",
                "eanGeoAirports");
        geoParentRegionsDataFormat.setEncoding(Charset.forName("UTF-8"));
        ...
        from("direct:processEanGeoParentRegions").routeId(EAN_GEO_PARENT_REGION_FEED_ROUTE)
                ...
                .beanRef("eanGeoTransformer", "toCanonicalRegions")
		...
		from("direct:processEanGeoAirports").routeId(EAN_GEO_AIRPORT_FEED_ROUTE)
				.errorHandler(loggingErrorHandler(log)).unmarshal(geoAirportsDataFormat)
				.log(LoggingLevel.INFO, "Processing EAN airports")
		...
		from("direct:processEanGeoNeighborhoodArea").routeId(EAN_GEO_NEIGHBORHOOD_FEED_ROUTE).errorHandler(loggingErrorHandler(log))
				.unmarshal(geoNeighborhoodsDataFormat).log(LoggingLevel.INFO, "Processing EAN Neighborhoods")

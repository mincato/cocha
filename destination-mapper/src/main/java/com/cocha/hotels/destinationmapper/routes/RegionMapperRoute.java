...
public class RegionMapperRoute extends SpringRouteBuilder {
    ...
    public void configure() throws Exception {
        from("{{mapper.consumer.uri}}")
                ...
                .to("sql:select distinct(countryCode) from Hotel?dataSource=#contentDataSource")
                .split(body())
                .transform()
                .simple("${body[countryCode]}")
                .log(LoggingLevel.INFO, "processing Region Mapping STARTED :country code: "+"${body}")
                .bean(hotelContentRepository,"findByCountryCode")                
                .bean(providersProcessor)
                .split().method(ProviderHashSplitter.class, "splitBody")
                .choice()
                .when(simple("${in.header.providers} contains 'EAN'")).to("direct:singleEANHotelChannel")
		        .when(simple("${in.header.providers} contains 'BKG'")).to("direct:singleBKGHotelChannel")
        			.otherwise().log(LoggingLevel.INFO, "Run Region Hotel Mapper : supplierCode not supported");                

...
public class HotelMapperRouteTest extends CamelSpringTestSupport {
	...
	@Mock private MapperProcessor mapperProcessor;
	
	@InjectMocks private HotelMapperRoute hotelMapperRoute;
	...
	public void test() throws Exception {
		 NotifyBuilder notify = new NotifyBuilder(context)
         	.from("jpaContent:" + HotelMapping.class.getName() + "?entityType=java.util.ArrayList&transactionManager=#contentTransactionManager&usePersist=true")
    ...
    private void assertEntityInDB() throws Exception {
        JpaEndpoint endpoint = (JpaEndpoint) context
                .getEndpoint("jpaContent:" + HotelMapping.class.getName() + "?entityType=java.util.ArrayList&transactionManager=#contentTransactionManager&usePersist=true");
        EntityManager em = endpoint.getEntityManagerFactory().createEntityManager();

        List<HotelMapping> list = em.createQuery("select x from com.cocha.hotels.model.content.mapping.HotelMapping x order by x.hotelId")
                .getResultList();
